...
// Block XAMPP dashboard paths at root level
$xamppPaths = ['/dashboard', '/xampp', '/webalizer', '/security', '/licenses', '/application'];
foreach ($xamppPaths as $xamppPath) {
    // Match if URI starts with XAMPP path (but allow ITE311-SIGNAR app dashboards)
    if (stripos($requestUri, $xamppPath) === 0 || preg_match('#^/' . preg_quote($xamppPath, '#') . '(/|$)#i', $requestUri)) {
        // Allow if it's part of our legitimate app routes
        if (!preg_match('#/ITE311-SIGNAR/(admin|student|teacher)/dashboard#i', $requestUri)) {
            // Redirect to login page
            header('Location: /ITE311-SIGNAR/login', true, 301);
            exit;
        }
    }
}
